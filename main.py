...
gravator = Gravatar(app, size=100, rating='g', default='reto',
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if isinstance(current_user, AnonymousUserMixin) or current_user.id != 1:
            return abort(403)
...
def register():
    ...
    if form.validate_on_submit():
        name = form.name.data
        hashed_and_salted_password = generate_password_hash(password=form.password.data, salt_length=8)
